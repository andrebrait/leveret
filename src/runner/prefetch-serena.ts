...
import { safeToolEnvironment, serenaPrefetchFixtures } from "./serena.js";
...
export async function prefetchSerena(options: PrefetchOptions): Promise<void> {
  ...
  await writeFile(
    ...
    stringify({
      ...
      // Runtime project.yml is generated by Leveret after refusing any checkout-owned .serena directory.
      trusted_project_path_patterns: ["**"],
      projects: [],
    }),
  );
  const root = await mkdtemp(join(tmpdir(), "leveret-serena-prefetch-"));
  const env = safeToolEnvironment({ ...process.env, SERENA_HOME: options.home });
  try {
